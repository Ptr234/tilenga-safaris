...
export default function KikorongoSafariLodgePage() {
  return (
    <>
      ...
      {/* ── GALLERY ── */}
      <section className="bg-forest-dark py-20 md:py-32">
        <div className="px-5 md:px-20 mb-16">
          <FadeIn>
            <p className="section-label text-gold mb-3">Gallery</p>
            <h2 className="font-serif text-4xl md:text-5xl text-cream">Lodge Life</h2>
          </FadeIn>
        </div>
        
        <StaggerGrid className="grid grid-cols-2 md:grid-cols-4 gap-2 px-2">
          {[
            "/photos/kikorongo fireplace.jpg",
            "/photos/kikorongo outside.jpg",
            "/photos/kikorongo ranger.jpg",
            "/photos/kikorongo travel.jpg",
          ].map((img, i) => (
            <StaggerItem key={i} className={i === 1 ? "md:col-span-2 md:row-span-2" : ""}>
              <div className="relative h-full min-h-[300px] overflow-hidden group">
                ...
                />
                <div className="absolute inset-0 bg-forest-dark/20 group-hover:bg-transparent transition-colors duration-500" />
              </div>
            </StaggerItem>
          ))}
        </StaggerGrid>
