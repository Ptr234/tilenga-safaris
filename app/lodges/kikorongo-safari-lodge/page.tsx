...
const rooms = [
  {
    ...
    count: "18 Units",
    description: "Named in the Lukhonzo language of Kasese, each cottage captures wide views of both Lake Kikorongo and Lake George — private terraces perfectly positioned for sunset watching over the Rwenzori Mountains.",
  ...
  },
  {
    name: "Premium Tents",
    count: "13 Units",
    description: "Tented accommodation that brings you closest to nature — bird calls at dawn, the lake shimmering below. Ideal for birders and photographers seeking complete immersion in Queen Elizabeth NP.",
  ...
  },
  {
    ...
    count: "5 Units",
    description: "Smaller, ground-level cottages suited to guests who prefer not to hike. Cozy and private, these units offer a quieter retreat within the lodge grounds with direct garden access.",
...
export default function KikorongoSafariLodgePage() {
  return (
    <>
      ...
      <section className="relative h-screen min-h-[600px] overflow-hidden flex items-end">
        ...
        <div className="relative z-10 px-8 md:px-20 pb-20 max-w-3xl">
          ...
          <h1 className="font-serif text-5xl md:text-7xl lg:text-8xl text-cream leading-none mb-4">
            Kikorongo<br /><em className="not-italic italic text-gold">Safari Lodge</em>
          </h1>
        ...
        <div className="absolute bottom-8 right-8 md:right-16 z-10 hidden md:flex gap-6">
          {quickFacts.map((f) => (
      ...
      {/* Lodge intro — editorial split */}
      <section className="bg-cream grid md:grid-cols-2 min-h-[70vh]">
        ...
        <div className="grid grid-cols-2 gap-1.5 p-1.5">
          <img src="https://images.unsplash.com/photo-1493246507139-91e8fad9978e?w=600&q=80" alt="Lake view" className="w-full h-56 md:h-full object-cover" />
          <div className="flex flex-col gap-1.5">
            <img src="https://images.unsplash.com/photo-1516426122078-c23e76319801?w=400&q=80" alt="Landscape" className="w-full h-28 object-cover flex-1" />
            <img src="https://images.unsplash.com/photo-1564760055775-d63b17a55c44?w=400&q=80" alt="Wildlife" className="w-full h-28 object-cover flex-1" />
            <img src="https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=400&q=80" alt="Lodge" className="w-full h-28 object-cover flex-1" />
          </div>
        </div>
      </section>

      {/* Accommodation — editorial side-by-side */}
          ...
          {rooms.map((room, i) => (
            ...
            >
              <div className="relative overflow-hidden h-64 md:h-auto group">
                ...
                />
                <div className="absolute top-5 left-5 font-serif text-cream/10 text-7xl leading-none select-none">{String(i + 1).padStart(2, "0")}</div>
                <span className="absolute top-5 right-5 border border-gold/40 text-gold text-[10px] uppercase tracking-widest font-sans px-3 py-1 bg-forest-dark/50 backdrop-blur-sm">
                  {room.count}
                </span>
              </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-end gap-8 mb-16">
            <div>
              ...
              <h2 className="font-serif text-4xl md:text-5xl text-forest leading-tight">Activities &amp;<br />Experiences</h2>
            </div>
            <div className="flex-1 h-px bg-gold/20 mb-3 hidden md:block" />
            <Link href="/plan-a-trip" className="hidden md:inline-block btn-primary shrink-0">Plan Your Stay</Link>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-0">
            {activities.map((act, i) => (
              <div key={act.name} className="group py-7 border-t border-gold/15 hover:border-gold/40 transition-colors duration-300">
                <p className="font-serif text-5xl text-gold/20 group-hover:text-gold/40 transition-colors duration-400 leading-none mb-4">
                  {String(i + 1).padStart(2, "0")}
                </p>
                <h3 className="font-serif text-xl text-forest mb-2 group-hover:text-forest-light transition-colors duration-300">{act.name}</h3>
      ...
      <div className="flex gap-0.5 h-52 md:h-72 overflow-hidden">
        {[
          ...
          <div key={i} className="flex-1 overflow-hidden group">
            <img src={src} alt={`Kikorongo gallery ${i + 1}`} className="w-full h-full object-cover transition-transform duration-[1200ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-110" />
          </div>
        ))}
      </div>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
            ...
            <ul className="space-y-5 mb-10">
              {[
                ...
                <li key={item.label} className="flex gap-4 text-sm">
                  ...
                  {item.href ? (
                    <a href={item.href} className="text-cream/70 font-sans hover:text-gold transition-colors" target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noopener noreferrer" : undefined}>{item.value}</a>
                  ) : (
          ...
          <div className="bg-forest-dark/60 border border-white/10 p-10">
            ...
            <div className="space-y-3">
              <Link href="/plan-a-trip" className="btn-outline w-full text-center block">Request a Proposal</Link>
              <a
